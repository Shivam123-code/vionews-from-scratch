      ...
      <main className="container py-8">
        ...
        {query && (
          <p className="text-muted-foreground mb-6">
            {isLoading ? "Searching..." : error ? "Error loading results" : `${searchResults?.length || 0} result${(searchResults?.length || 0) !== 1 ? "s" : ""} for "${query}"`}
          </p>
        )}
          ...
          <div className="lg:col-span-2">
            {isLoading ? (
              ...
              searchResults && searchResults.length > 0 ? (
                <div className="space-y-4">{searchResults.map((article) => <ArticleCard key={article.id} article={article} />)}</div>
              ) : (
              ...
              <div className="text-center py-16 bg-card rounded-lg">
                <Search className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                <h2 className="text-xl font-bold mb-2">Search VioNews</h2>
                <p className="text-muted-foreground">Enter keywords to find news articles</p>
              </div>
            )}
          </div>
